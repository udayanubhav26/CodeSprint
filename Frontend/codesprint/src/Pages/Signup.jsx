...
const signupSchema = z.object({
    firstName:z.string().min(3,"Name should be atleast 3 char!"),
    ...
    return(
        <div className="min-h-screen w-full flex justify-center items-center bg-base-200">
          <form onSubmit={handleSubmit(submittedData)} className="bg-base-100 shadow-2xl rounded-xl p-8 w-full max-w-md flex flex-col gap-6">
            ...
            <div>
               <input className="input input-bordered w-full" {...register("firstName")} placeholder="Anubhav Sharma"/>
               {errors.firstName && (<span className="text-red-500 text-sm">{errors.firstName.message}</span>)} {/*&& is used if first is true then second is printed otherwise second is printed!*/}
            </div>
            ...
            <button type='submit' className="btn btn-primary w-full bg-purple-600 border border-purple-700">Sign Up</button>
